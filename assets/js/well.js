...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const animateElements = document.querySelectorAll('.hover-lift, .crisis-card, .expertise-card, .speaker-card, .topic-card, .outcome-card, .approach-card, .pricing-card');
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', function(e) {
            ...
            setTimeout(() => {
                ...
                submitBtn.innerHTML = '<i class="fas fa-check me-2"></i>Consultation Scheduled!';
    ...
    speakerCards.forEach(card => {
        card.addEventListener('mouseenter', function() {
            const topics = this.querySelectorAll('.speaker-topics li');
            topics.forEach((topic, index) => {
                setTimeout(() => {
                    topic.style.transform = 'translateX(5px)';
                    topic.style.color = 'var(--electric-cyan)';
                }, index * 100);
            });
        });
    ...
    // Wellness chart animation enhancement
    const wellnessChart = document.querySelector('.wellness-chart');
    if (wellnessChart) {
        const chartObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
        ...
        // Set initial state
        const bars = wellnessChart.querySelectorAll('.bar-fill');
        bars.forEach(bar => {
            bar.style.opacity = '0';
        ...
        chartObserver.observe(wellnessChart);
    }
        ...
        card.addEventListener('mouseenter', function() {
            ...
            details.forEach((detail, index) => {
                setTimeout(() => {
                    ...
                    detail.style.color = 'var(--electric-cyan)';
                }, index * 100);
            });
        });
    ...
    pricingCards.forEach(card => {
        card.addEventListener('mouseenter', function() {
            ...
            this.style.boxShadow = '0 15px 30px rgba(0, 217, 255, 0.2)';
        });
    ...
    console.log('Wellness & Mental Health Speaking page initialized successfully! 🌱');
});
